...
class App extends Component {
  ...
  componentDidMount() {
    fetch(
      `https://api.discogs.com/labels/528464/releases?page=1&per_page=100&key=${
        process.env.REACT_APP_TMDB_API_KEY
      }&secret=${process.env.REACT_APP_TMDB_API_SECRET}`
    )
      .then(res => res.json())
      .then(json => {
          ...
          <div>
            ...
            <div>
              ...
              <Route
                path="/album"
                render={routeProps => (
                  <Album {...routeProps} releaseUrl={this.state.releaseUrl} />
                )}
              />
            </div>
          </div>
